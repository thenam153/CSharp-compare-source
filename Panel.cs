...
public class Panel : IActionListener, IChatable
{
	...
	public string[][][] tabName = new string[26][][]
	{
		null,
		null,
		boxTabName,
	...
	private int[] zoneColor = new int[3]
	{
		43520,
		14743570,
		14155776
	...
	public void addItemDetail(Item item)
	{
		...
		if (item.itemOption != null)
		{
			for (int i = 0; i < item.itemOption.Length; i++)
			{
				...
				{
					str2 = " [+" + item.itemOption[i].param + "]";
				}
			}
		}
		str = str + "|0|1|" + item.template.name + str2;
		if (item.itemOption != null)
		{
		...
		{
			string str3 = mResources.pow_request + ": " + currItem.template.strRequire;
	...
	public void addMessageDetail(ClanMessage cm)
	{
		...
		for (int i = 0; i < myMember.size(); i++)
		{
			...
			{
				string text = str;
				str = text + "\n|4|" + mResources.clan_point + ": " + member.clanPoint;
				text = str;
				str = text + "\n|5|" + mResources.give_pea + ": " + member.donate + mResources.time;
				text = str;
				str = text + "\n|5|" + mResources.receive_pea + ": " + member.receive_donate + mResources.time;
				partID = new int[3]
				{
					member.head,
					member.leg,
	...
	{
		string arg = "|0|1|" + t.name;
		arg = arg + "\n|1|Top " + t.rank;
		arg = arg + "\n|1|" + t.info;
		arg = arg + "\n|2|" + t.info2;
		cp = new ChatPopup();
		popUpDetailInit(cp, arg);
		partID = new int[3]
		{
			t.headID,
			t.leg,
	...
	public void addClanMemberDetail(Member m)
	{
		...
		str = str + str2 + Member.getRole(m.role);
		string text = str;
		str = text + "\n|2|1|" + mResources.power + ": " + m.powerPoint;
		str += "\n--";
		text = str;
		str = text + "\n|5|" + mResources.give_pea + ": " + m.donate + mResources.time;
		text = str;
		str = text + "\n|5|" + mResources.receive_pea + ": " + m.receive_donate + mResources.time;
		text = str;
		str = text + "\n|4|" + mResources.clan_point + ": " + m.clanPoint;
		text = str;
	...
	private void paintShop(mGraphics g)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (item != null)
			{
				...
				if (item.buyCoin != 0 || item.buyGold != 0)
				{
					if (typeShop != 2 && item.powerRequire == 0)
					{
						if (item.buyCoin > 0)
						{
							mFont.tahoma_7_green.drawString(g, Res.formatNumber(item.buyCoin) + " " + mResources.XU, num2 + num4 - 2, num3 + 1, mFont.RIGHT);
						}
						if (item.buyGold > 0)
						{
							mFont.tahoma_7_green.drawString(g, Res.formatNumber(item.buyGold) + " " + mResources.LUONG, num2 + num4 - 2, num3 + 1, mFont.RIGHT);
						}
					}
					if (typeShop == 2 && currentTabIndex <= 3 && !Equals(GameCanvas.panel2))
					{
						if (item.buyCoin > 0)
						{
							mFont.tahoma_7_green.drawString(g, Res.formatNumber2(item.buyCoin), num2 + num4 - 2, num3 + 1, mFont.RIGHT);
						}
						if (item.buyGold > 0)
						{
							mFont.tahoma_7b_green.drawString(g, Res.formatNumber2(item.buyGold) + " " + mResources.LUONG, num2 + num4 - 2, num3 + 1, mFont.RIGHT);
						}
					}
				}
				SmallImage.drawSmallImage(g, item.template.iconID, num5 + num7 / 2 - ((item.quantity > 1) ? 8 : 0), num6 + num8 / 2, 0, 3);
			...
			if (item.buyType == 1)
			{
				...
				if (item.buyCoin != -1)
				{
					mFont.tahoma_7_green.drawString(g, Res.formatNumber2(item.buyCoin) + " " + mResources.XU, num2 + num4 - 5, num3 + 13, mFont.RIGHT);
				}
				else if (item.buyGold != -1)
				{
					mFont.tahoma_7_green.drawString(g, Res.formatNumber2(item.buyGold) + " " + mResources.LUONG, num2 + num4 - 5, num3 + 11, mFont.RIGHT);
				}
			}
			else if (item.buyType == 2)
			{
				...
				if (item.buyCoin != -1)
				{
					mFont.tahoma_7b_blue.drawString(g, Res.formatNumber2(item.buyCoin) + " " + mResources.XU, num2 + num4 - 5, num3 + 11, mFont.RIGHT);
				}
				else if (item.buyGold != -1)
				{
					mFont.tahoma_7b_blue.drawString(g, Res.formatNumber2(item.buyGold) + " " + mResources.LUONG, num2 + num4 - 5, num3 + 11, mFont.RIGHT);
				}
			}
		}
		paintScrollArrow(g);
	...
	private void paintArchivement(mGraphics g)
	{
		...
		else
		{
			...
			for (int i = 0; i < currentListLength; i++)
			{
				...
				{
					mFont.tahoma_7.drawString(g, archivement.info1, num + 5, num2, 0);
					mFont.tahoma_7_blue.drawString(g, mResources.reward_mission + archivement.money + " " + mResources.LUONG, num + 5, num2 + 11, 0);
					if (i == selected)
					{
	...
	private void paintCombine(mGraphics g)
	{
		...
		for (int j = 0; j < vItemCombine.size() + 1; j++)
		{
			...
			if (item != null)
			{
				if (item.isHaveOption(34))
				{
					g.setColor((j != selected) ? color1[0] : color2[0]);
				}
				else if (item.isHaveOption(35))
				{
					g.setColor((j != selected) ? color1[1] : color2[1]);
				}
				else if (item.isHaveOption(36))
				{
					g.setColor((j != selected) ? color1[2] : color2[2]);
				}
			}
			g.fillRect(num5, num6, num7, num8);
			if (item == null)
	...
	private void paintTab(mGraphics g)
	{
		...
		for (int i = 0; i < currentTabName.Length; i++)
		{
			...
			mFont mFont = (i != currentTabIndex) ? mFont.tahoma_7_grey : mFont.tahoma_7_green2;
			if (!currentTabName[i][1].Equals(string.Empty))
			{
				mFont.drawString(g, currentTabName[i][0], startTabPos + i * TAB_W + TAB_W / 2, 53, mFont.CENTER);
				mFont.drawString(g, currentTabName[i][1], startTabPos + i * TAB_W + TAB_W / 2, 64, mFont.CENTER);
			}
			else
			{
				mFont.drawString(g, currentTabName[i][0], startTabPos + i * TAB_W + TAB_W / 2, 59, mFont.CENTER);
			}
	...
	private void paintBottomMoneyInfo(mGraphics g)
	{
		if (type != 13 || (currentTabIndex != 2 && !Equals(GameCanvas.panel2)))
		{
			...
			g.drawImage(imgXu, X + 21, H - 7, 3);
			g.drawImage(imgLuong, X + 100, H - 8, 3);
			mFont.tahoma_7_yellow.drawString(g, Char.myCharz().xu + string.Empty, X + 34, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
			mFont.tahoma_7_yellow.drawString(g, Char.myCharz().luong + string.Empty, X + 110, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
		}
	}
			...
			if (Char.myCharz().taskMaint != null)
			{
				...
				for (int j = 0; j < Char.myCharz().taskMaint.subNames.Length; j++)
				{
					...
					else if (Char.myCharz().taskMaint.index <= j)
					{
						...
						mFont.drawString(g, empty, xstart + 5 + ((mFont == mFont.tahoma_7_blue && GameCanvas.gameTick % 20 > 10) ? (GameCanvas.gameTick % 4 / 2) : 0), yPaint += 12, 0);
					}
				}
				yPaint += 5;
	...
	private void addFriend(InfoItem info)
	{
		...
		str += "\n";
		str = ((!info.isOnline) ? (str + "|3|1|" + mResources.is_offline) : (str + "|4|1|" + mResources.is_online));
		str += "\n--";
		string text = str;
	...
	public void perform(int idAction, object p)
	{
		...
		if (idAction == 3002)
		{
			GameCanvas.endDlg();
			Item item12 = (Item)p;
			Service.gI().saleItem(0, (sbyte)((selected > 6) ? 1 : 0), (short)((selected >= 7) ? (selected - 6 - 1) : selected));
		}
		if (idAction == 3003)
		{
			GameCanvas.endDlg();
	...
	public void onChatFromMe(string text, string to)
	{
		...
		else if (chatTField.strChat.Equals(mResources.input_clan_name))
		{
			InfoDlg.showWait();
			chatTField.isShow = false;
			Service.gI().searchClan(text);
		}
		else if (chatTField.strChat.Equals(mResources.chat_clan))
		{
			InfoDlg.showWait();
			chatTField.isShow = false;
			Service.gI().clanMessage(0, text, -1);
		}
		else if (chatTField.strChat.Equals(mResources.input_clan_name_to_create))
		{
		...
		else if (chatTField.strChat.Equals(mResources.input_clan_slogan))
		{
			if (chatTField.tfChat.getText() == string.Empty)
			{
		...
		else if (chatTField.strChat.Equals(mResources.input_Inventory_Pass))
		{
			try
			{
		...
		else if (chatTField.strChat.Equals(mResources.world_channel_5_luong))
		{
		...
		{
			Service.gI().kigui(0, currItem.itemId, 1, int.Parse(chatTField.tfChat.getText()), 1);
			chatTField.isShow = false;
		}
		else if (chatTField.strChat.Equals(mResources.kiguiLuongchat + "  "))
		{
			Service.gI().kigui(0, currItem.itemId, 1, int.Parse(chatTField.tfChat.getText()), (sbyte)currItem.quantilyToBuy);
			chatTField.isShow = false;
		}
	...
	private void doFireOption()
	{
		...
		case 1:
			if (!Main.isPC)
			{
				...
				{
					strCauhinh[1] = mResources.turnOffAnalog;
					GameScr.isAnalog = 1;
				...
				{
					strCauhinh[1] = mResources.turnOnAnalog;
					GameScr.isAnalog = 0;
					Rms.saveRMSInt("analog", GameScr.isAnalog);
					GameScr.setSkillBarPosition();
				}
