...
public class Mob : IMapObject
{
	...
	public static void removeBigBoss()
	{
		...
		while (true)
		{
			if (num < GameScr.vMob.size())
			{
				mob = (Mob)GameScr.vMob.elementAt(num);
				if (mob is BigBoss)
				{
					break;
				}
	...
	private void updateInjure()
	{
		...
		timeStatus--;
		if (timeStatus <= 0 && (isNewModStand() || (isNewMod() && frame == 11) || (isSpecial() && frame == 15) || (templateId < 58 && frame == 11)))
		{
	...
	public void updateMobAttack()
	{
		if (tick < 2)
		{
			checkFrameTick((p3 != 0) ? attack2 : attack1);
		...
		else if (p1 == 1)
		{
			...
			if (Res.abs(xFirst - x) < 5 && Res.abs(yFirst - y) < 5 && tick == 2)
			{
				status = 2;
				p1 = 0;
				p2 = 0;
