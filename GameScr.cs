...
public class GameScr : mScreen, IChatable
{
	...
	public static void loadCamera(bool fullmScreen, int cx, int cy)
	{
		...
		{
			GameCanvas.panel.isViewChatServer = ((Rms.loadRMSInt("viewchat") == 1) ? true : false);
		}
	}
	...
	public override void paint(mGraphics g)
	{
		...
		for (int i = 0; i < vCharInMap.size(); i++)
		{
			...
			if (char4 != null && char4.skillPaint != null && char4.skillInfoPaint() != null && char4.indexSkill < char4.skillInfoPaint().Length)
			{
		...
		if (isPaintRada == 1 && !GameCanvas.panel.isShow)
		{
			paintInfoBar(g);
		}
		resetTranslate(g);
		...
		else
		{
			...
			if (isHaveSelectSkill)
			{
				...
				for (int i = 0; i < num4; i++)
				{
					if (Main.isPC)
					{
						string[] array2 = TField.isQwerty ? new string[5]
						{
							"1",
							"2",
							"3",
