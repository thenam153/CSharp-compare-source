	...
	public void paintIcon(mGraphics g)
	{
		...
		for (int i = 0; i < nItem; i++)
		{
			...
			if (num2 + WIDTH - ((scrMain != null) ? scrMain.cmy : 0) >= y + disStart && num2 - ((scrMain != null) ? scrMain.cmy : 0) <= y + disStart + h)
			{
				...
				if (clanImage2.xu > 0)
				{
					mFont.drawString(g, clanImage2.xu + " " + mResources.XU, num + w - 20, num2, mFont.RIGHT);
				}
				else if (clanImage2.luong > 0)
				{
					mFont.drawString(g, clanImage2.luong + " " + mResources.LUONG, num + w - 20, num2, mFont.RIGHT);
				}
				else
				{
					mFont.drawString(g, mResources.free, num + w - 20, num2, mFont.RIGHT);
				}
