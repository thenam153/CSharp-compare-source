...
public class Char : IMapObject
{
	...
	{
		if (isCopy || clevel >= 14 || isFusion || isSetPos || isPet || isMiniPet || isMonkey == 1)
		{
			return;
		}
		ty++;
	...
	public void paintCharBody(mGraphics g, int cx, int cy, int cdir, int cf, bool isPaintBag)
	{
		...
		if (statusMe == 14)
		{
			...
			SmallImage.drawSmallImage(g, 834, cx, cy - CharInfo[cf][2][2] + part3.pi[CharInfo[cf][2][0]].dy - 2 + num2, transform, StaticObj.TOP_CENTER);
			SmallImage.drawSmallImage(g, 79, cx, cy - ch - 8, 0, mGraphics.HCENTER | mGraphics.BOTTOM);
			SmallImage.drawSmallImage(g, part.pi[CharInfo[cf][0][0]].id, cx + (CharInfo[cf][0][1] + part.pi[CharInfo[cf][0][0]].dx) * num, cy - CharInfo[cf][0][2] + part.pi[CharInfo[cf][0][0]].dy, transform, anchor);
			if (head == 542 && GameCanvas.gameTick % 10 > 5)
			{
				Part part4 = GameScr.parts[543];
				SmallImage.drawSmallImage(g, part4.pi[CharInfo[cf][0][0]].id, cx + (CharInfo[cf][0][1] + part4.pi[CharInfo[cf][0][0]].dx) * num, cy - CharInfo[cf][0][2] + part4.pi[CharInfo[cf][0][0]].dy, transform, anchor);
			}
		}
		else
		{
			...
			if (head == 542 && GameCanvas.gameTick % 10 > 5)
			{
				...
				SmallImage.drawSmallImage(g, part5.pi[CharInfo[cf][0][0]].id, cx + (CharInfo[cf][0][1] + part5.pi[CharInfo[cf][0][0]].dx) * num, cy - CharInfo[cf][0][2] + part5.pi[CharInfo[cf][0][0]].dy, transform, anchor);
			}
			SmallImage.drawSmallImage(g, part2.pi[CharInfo[cf][1][0]].id, cx + (CharInfo[cf][1][1] + part2.pi[CharInfo[cf][1][0]].dx) * num, cy - CharInfo[cf][1][2] + part2.pi[CharInfo[cf][1][0]].dy, transform, anchor);
			SmallImage.drawSmallImage(g, part3.pi[CharInfo[cf][2][0]].id, cx + (CharInfo[cf][2][1] + part3.pi[CharInfo[cf][2][0]].dx) * num, cy - CharInfo[cf][2][2] + part3.pi[CharInfo[cf][2][0]].dy, transform, anchor);
		}
		...
		{
			g.drawImage((cgender != 1) ? eyeTraiDat : eyeNamek, cx + -((cgender != 1) ? 2 : 2) * num, cy - 32 + ((cgender != 1) ? 11 : 10) - cf, anchor2);
		}
		if (eProtect != null)
		{
			eProtect.paint(g);
	...
	{
		if (Res.abs(toX - cx) > 100 || Res.abs(toY - cy) > 300)
		{
